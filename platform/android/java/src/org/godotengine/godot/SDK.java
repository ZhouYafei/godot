		...
		U8SDK.getInstance().runOnMainThread(new Runnable() {
			@Override public void run() {
				...
				if(type.equals("start_level"))
					...
					// public void onEventValue(String eventId, String paramString, Map<String, String> paramMap, int paramInt);
					String eventId = data.getString("event_id");
					String paramString = data.getString("param_string");
					String paramMap = dict_to_map(data.getDictionary("param_map"));
					int paramInt = data.getInt("param_int");
					U8Analytics.getInstance().onEvent(eventId, paramString, paramMap, paramInt);
				}
